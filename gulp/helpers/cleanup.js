...
export default function cleanUp(handler) {
  ...
  originalCleanUp((exitCode, signal) => {
    ...
    if (!handlers.has(handler)) {
      // Cleanup for handler is complete
      return exit;
    }
...
function exitOnlyIfLastChild(signal, handler) {
  if (called.has(handler)) {
    // Cleanup already called for handler
    return;
  }
...
function unlock() {
  const lockFiles = path.join(process.cwd(), buildDir, `.lock-gulp-tdd-*`);
  execSync(`rm -f ${lockFiles}`);
}

function getLockFiles() {
  return execSync(
    `ls -al ${path.join(process.cwd(), buildDir)} | grep ${".lock-gulp-tdd"}`
  )
    .toString()
    .split(/[ \t\r\n]+/)
    .filter((str) => {
...
function isTopProcess() {
  try {
    fs.statSync(
      path.join(process.cwd(), buildDir, `.lock-gulp-tdd-${process.ppid}`)
    );
    return false;
  } catch (e) {
    return true;
